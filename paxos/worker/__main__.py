...
class Worker:
    ...
    def make_flask_app(self):
        ...
        def transfer_funds():
            ...
            self.ledger.transfer(from_uid=data["from_uid"], to_uid=data["to_uid"], amount=data["amount"],)
            return {}
    ...
    #     comm = web.Application()
    #     comm.add_routes([web.post("/", handle)])
    ...
    def main(self):
        ...
        flask_thr = Thread(target=lambda: flask_app.run(use_reloader=False, debug=False, port=self.args.flask_port,), daemon=True)

        # comm_srv = self.make_comm_server()
        # comm_thr = Thread(target=lambda: web.run_app(comm_srv, port=self.args.comm_port))
